...
Normalise `oxy[haem]` (O2Hb) and `deoxy[haem]` (HHb) channels for blood-volume changes (method developed from *Beever & Tripp et al, 2020*). Specify at least two of the oxy, deoxy, and total (THb) channels; the third is derived. After correction, `total[haem]` is definitionally zero.

#### Place Event Markers:
Manually add event markers at specified time points. Will add an `event_channel` to the data table if not already specified. If the existing `event_channel` holds character labels, markers are written there; if it holds integer lap numbers, it is left unchanged and markers are written to a new `event_labels` column placed after it (*By Label* on the Extract page then matches `event_labels`).
